/* Deterministic classifier, ported from ~/Documents/trustlayer
   `lib/ai/heuristic.ts`. This is the no-API-key path the source repo
   ships, and the one its `npm run bench` uses, so the demo reaches the
   same decisions as the benchmark quoted in the case study. */

import { SIDE_EFFECTING_TASKS as SIDE_EFFECTING_TASK_TYPES } from "./config";
import type {
  Classification,
  Domain,
  EvidenceStatus,
