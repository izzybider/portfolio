/* ============================================================
   TRUSTLAYER SIMULATED TOOLS — ported from
   ~/Documents/trustlayer `lib/tools/index.ts`.

   The six handlers, their fixtures, their summaries and the
   state-derived tool planner are unchanged. The only substitution is
   argument validation: that repo uses zod, and this one enforces the
   same required/length/character-set rules with a small local checker
   so the portfolio adds no dependency for a demo that only ever reads
   its own fixtures.
   ============================================================ */
...
import type { StructuredState, ToolCall, ToolResult, ToolName } from "./types";
...
/**
 * Argument validation. The source repo expresses this with zod; the same
 * three constraints are enforced here without the dependency — required,
 * length-bounded, and restricted to a safe character set — so a tool can
 * never be handed an argument it did not declare.
 */
type ArgSpec = { required: string[]; optional?: string[]; anyOf?: string[] };
...
function checkField(field: string, raw: unknown): string | null {
  if (typeof raw !== "string") return `${field} is required`;
  const value = raw.trim();
  if (value.length === 0) return `${field} is required`;
  if (value.length > 64) return `${field} is too long`;
  if (!FIELD_PATTERN.test(value)) return `${field} contains unsupported characters`;
  return null;
}

function validateArgs(spec: ArgSpec, args: Record<string, string>): ArgCheck {
  const value: Record<string, string> = {};
  for (const field of spec.required) {
    const problem = checkField(field, args[field]);
    if (problem) return { ok: false, error: problem };
    value[field] = String(args[field]).trim();
  }
  ...
  if (spec.anyOf && !spec.anyOf.some((field) => value[field])) {
    return { ok: false, error: `${spec.anyOf.join(" or ")} is required` };
  }
...
const lookupReservation: ToolHandler = {
  ...
  args: { required: [], optional: ["reservation_id", "guest"], anyOf: ["reservation_id", "guest"] },
  run: (args) => {
...
const getProductMetrics: ToolHandler = {
  ...
  args: { required: ["metric"], optional: ["date"] },
  run: (args) => {
...
const checkAuthorization: ToolHandler = {
  ...
  args: { required: ["action"], optional: ["role", "amount_usd"] },
  run: (args) => {
...
export function runTool(call: ToolCall): ToolResult {
  ...
  const parsed = validateArgs(handler.args, call.args);
  if (!parsed.ok) {
    return {
      call_id: call.id,
      tool: call.tool,
      status: "error",
      summary: `Invalid arguments: ${parsed.error}`,
      latency_ms: Date.now() - started,
    };
  }
  try {
    const out = handler.run(parsed.value);
